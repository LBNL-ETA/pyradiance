 ...
 *l'Energie Centre de Valbonne, 500 route des Lucioles, 06565 Sophia Antipolis
 *Cedex, France *BOUYGUES 1 Avenue Eugene Freyssinet, Saint-Quentin-Yvelines,
 *France print colored output if activated in command line (-C). Based on model
 *from A. Diakite, TU-Berlin. Implemented by J. Wienold, August 26 2018
 *  version 2.6 (2021/01/29): dew point dependency added according to Perez
 *publication 1990 (W -> atm_preci_water=exp(0.07*Td-0.075) ). by J. Wienold,
 *EPFL
 */
 ...
 * /home/cvsd/radiance/ray/src/gen/gendaylit.c,v 2.21 2021/01/28 19:03:15 greg
 * Exp $";*/

float coeff_perez[] = {
    1.3525,  -0.2576,  -0.2690,  -1.4366, -0.7670, 0.0007,   1.2734,   -0.1233,
    2.8000,  0.6004,   1.2375,   1.000,   1.8734,  0.6297,   0.9738,   0.2809,
    0.0356,  -0.1246,  -0.5718,  0.9938,  -1.2219, -0.7730,  1.4148,   1.1016,
    -0.2054, 0.0367,   -3.9128,  0.9156,  6.9750,  0.1774,   6.4477,   -0.1239,
    -1.5798, -0.5081,  -1.7812,  0.1080,  0.2624,  0.0672,   -0.2190,  -0.4285,
    -1.1000, -0.2515,  0.8952,   0.0156,  0.2782,  -0.1812,  -4.5000,  1.1766,
    24.7219, -13.0812, -37.7000, 34.8438, -5.0000, 1.5218,   3.9229,   -2.6204,
    -0.0156, 0.1597,   0.4199,   -0.5562, -0.5484, -0.6654,  -0.2672,  0.7117,
    0.7234,  -0.6219,  -5.6812,  2.6297,  33.3389, -18.3000, -62.2500, 52.0781,
    -3.5000, 0.0016,   1.1477,   0.1062,  0.4659,  -0.3296,  -0.0876,  -0.0329,
    -0.6000, -0.3566,  -2.5000,  2.3250,  0.2937,  0.0496,   -5.6812,  1.8415,
    21.0000, -4.7656,  -21.5906, 7.2492,  -3.5000, -0.1554,  1.4062,   0.3988,
    0.0032,  0.0766,   -0.0656,  -0.1294, -1.0156, -0.3670,  1.0078,   1.4051,
    0.2875,  -0.5328,  -3.8500,  3.3750,  14.0000, -0.9999,  -7.1406,  7.5469,
    -3.4000, -0.1078,  -1.0750,  1.5702,  -0.0672, 0.4016,   0.3017,   -0.4844,
    -1.0000, 0.0211,   0.5025,   -0.5119, -0.3000, 0.1922,   0.7023,   -1.6317,
    19.0000, -5.0000,  1.2438,   -1.9094, -4.0000, 0.0250,   0.3844,   0.2656,
    1.0468,  -0.3788,  -2.4517,  1.4656,  -1.0500, 0.0289,   0.4260,   0.3590,
    -0.3250, 0.1156,   0.7781,   0.0025,  31.0625, -14.5000, -46.1148, 55.3750,
    -7.2312, 0.4050,   13.3500,  0.6234,  1.5000,  -0.6426,  1.8564,   0.5636};

float defangle_theta[] = {
    84, 84, 84, 84, 84, 84, 84, 84, 84, 84, 84, 84, 84, 84, 84, 84, 84, 84, 84,
    84, 84, 84, 84, 84, 84, 84, 84, 84, 84, 84, 72, 72, 72, 72, 72, 72, 72, 72,
    72, 72, 72, 72, 72, 72, 72, 72, 72, 72, 72, 72, 72, 72, 72, 72, 72, 72, 72,
    72, 72, 72, 60, 60, 60, 60, 60, 60, 60, 60, 60, 60, 60, 60, 60, 60, 60, 60,
    60, 60, 60, 60, 60, 60, 60, 60, 48, 48, 48, 48, 48, 48, 48, 48, 48, 48, 48,
    48, 48, 48, 48, 48, 48, 48, 48, 48, 48, 48, 48, 48, 36, 36, 36, 36, 36, 36,
    36, 36, 36, 36, 36, 36, 36, 36, 36, 36, 36, 36, 24, 24, 24, 24, 24, 24, 24,
    24, 24, 24, 24, 24, 12, 12, 12, 12, 12, 12, 0};
...
float defangle_phi[] = {
    ...
    225, 240, 255, 270, 285, 300, 315, 330, 345, 0,   15,  30,  45,  60,  75,
    90,  105, 120, 135, 150, 165, 180, 195, 210, 225, 240, 255, 270, 285, 300,
    315, 330, 345, 0,   20,  40,  60,  80,  100, 120, 140, 160, 180, 200, 220,
    240, 260, 280, 300, 320, 340, 0,   30,  60,  90,  120, 150, 180, 210, 240,
    270, 300, 330, 0,   60,  120, 180, 240, 300, 0};
/* default values for Berlin */
float locus[] = {-4.843e9,  2.5568e6, 0.24282e3, 0.23258, -4.843e9, 2.5568e6,
                 0.24282e3, 0.23258,  -1.2848,   1.7519,  -0.093786};

/* Perez sky parametrization: epsilon and delta calculations from the direct and
 * diffuse irradiances */
double sky_brightness();
double sky_clearness();

/* calculation of the direct and diffuse components from the Perez
 * parametrization */
double diffuse_irradiance_from_sky_brightness();
double direct_irradiance_from_sky_clearness();

/* Perez global horizontal, diffuse horizontal and direct normal luminous
 * efficacy models : */
...
double calc_rel_lum_perez(double dzeta, double gamma, double Z, double epsilon,
                          double Delta, float coeff_perez[]);
void coeff_lum_perez(double Z, double epsilon, double Delta,
                     float coeff_perez[]);
double radians(double degres);
double degres(double radians);
void theta_phi_to_dzeta_gamma(double theta, double phi, double *dzeta,
...
const double skyclearinf =
    1.0; /* limitations for the variation of the Perez parameters */
...
double diffuseilluminance, directilluminance, diffuseirradiance,
    directirradiance, globalirradiance;
...
int main(int argc, char **argv) {
  ...
  for (i = 4; i < argc; i++)
    if (argv[i][0] == '-' || argv[i][0] == '+')
      ...
      case 'O':
        output =
            atof(argv[++i]); /*define the unit of the output of the program:
...
void computesky() {
  ...
  if (month) { /* from date and time */
    ...
    if (timeinterval) {
      ...
      if ((st < solar_sunrise(month, day) - timeinterval / 120) ||
          (st > solar_sunset(month, day) + timeinterval / 120)) {
        if (suppress_warnings == 0) {
          fprintf(stderr,
                  "Warning: sun position too low, printing error sky at %d %d "
                  "%.3f\n",
                  month, day, hour);
        }
        altitude = salt(sd, st);
      ...
      if (suppress_warnings == 0) {
        fprintf(stderr,
                "Warning: sun altitude below zero at time step %i %i %.2f, "
                "printing error sky\n",
                month, day, hour);
      }
      altitude = salt(sd, st);
  ...
  /* compute the inputs for the calculation of the light distribution over the
   * sky*/
  if (input == 0) /* P */
  {
    check_parametrization();
    diffuseirradiance =
        diffuse_irradiance_from_sky_brightness(); /*diffuse horizontal
                                                     irradiance*/
  ...
  else if (input == 3) /* G */
  {
    if (altitude <= 0) {
      if (suppress_warnings == 0)
        fprintf(stderr, "Warning: sun altitude < 0, proceed with irradiance "
                        "values of zero\n");
      directirradiance = 0;
      diffuseirradiance = 0;
    } else {
    ...
    if (output == 0 || output == 2) {
      ...
      check_illuminances();
           ...
           4) /* E */ /* Implementation of the Erbs model. W.Sprenger (04/13) */
  {

    if (altitude <= 0) {
      if (suppress_warnings == 0 && globalirradiance > 50)
        fprintf(stderr, "Warning: global irradiance higher than 50 W/m^2 while "
                        "the sun altitude is lower than zero\n");
      globalirradiance = 0;
      diffuseirradiance = 0;
      directirradiance = 0;
      ...
      if (globalirradiance > erbs_s0) {
        if (suppress_warnings == 0)
          fprintf(stderr, "Warning: global irradiance is higher than the "
                          "time-dependent solar constant s0\n");
      ...
      if (erbs_kt <= 0.22)
        diffuseirradiance = globalirradiance * (1 - 0.09 * erbs_kt);
      else if (erbs_kt <= 0.8)
        diffuseirradiance =
            globalirradiance *
            (0.9511 - 0.1604 * erbs_kt + 4.388 * pow(erbs_kt, 2) -
             16.638 * pow(erbs_kt, 3) + 12.336 * pow(erbs_kt, 4));
      else if (erbs_kt < 1)
        diffuseirradiance = globalirradiance * (0.165);
      ...
      printf("# WARNING: the -E option is only recommended for a rough "
             "estimation!\n");
    ...
    fprintf(stderr, "error at the input arguments");
    exit(1);
  }

  /* normalization factor for the relative sky luminance distribution, diffuse
   * part*/
  ...
  for (j = 0; j < 145; j++) {
    theta_phi_to_dzeta_gamma(radians(*(theta_o + j)), radians(*(phi_o + j)),
                             &dzeta, &gamma, radians(sunzenith));

    *(lv_mod + j) =
        calc_rel_lum_perez(dzeta, gamma, radians(sunzenith), skyclearness,
  ...
  /* integration of luminance for the normalization factor, diffuse part of the
   * sky*/
  ...
  if (output == 0) {
    ...
    fprintf(stderr, "Wrong output specification.\n");
    exit(1);
  }
  ...
  if (output == 0)
    solarradiance = directilluminance /
                    (2 * M_PI * (1 - cos(half_sun_angle * M_PI / 180))) /
                    WHTEFFICACY;

  else if (output == 1)
    solarradiance =
        directirradiance / (2 * M_PI * (1 - cos(half_sun_angle * M_PI / 180)));

  else
    solarradiance =
        directilluminance / (2 * M_PI * (1 - cos(half_sun_angle * M_PI / 180)));

  /* Compute the ground radiance */
  zenithbr = calc_rel_lum_perez(0.0, radians(sunzenith), radians(sunzenith),
                                skyclearness, skybrightness, coeff_perez);
  ...
  if (skyclearness >= 6) {
    F2 = 0.274 * (0.91 + 10.0 * exp(-3.0 * (M_PI / 2.0 - altitude)) +
  ...
  if ((skyclearness > 1) && (skyclearness < 6)) {
    ...
    F2 = (2.739 + .9891 * sin(.3119 + 2.6 * altitude)) *
         exp(-(M_PI / 2.0 - altitude) * (.4441 + 1.48 * altitude));
    normfactor = normsc() / F2 / M_PI;
  }
  ...
  if (*(c_perez + 1) > 0) {
    if (suppress_warnings == 0) {
      fprintf(
          stderr,
          "Warning: positive Perez parameter B (= %lf), printing error sky\n",
          *(c_perez + 1));
    }
    print_error_sky();
    exit(0);
...
void printsky() {
  ...
  printf("# Solar altitude and azimuth: %.1f %.1f\n", altitude * 180 / M_PI,
         azimuth * 180 / M_PI);
  printf("# epsilon, delta, atmospheric precipitable water content : %.4f %.4f "
         "%.4f \n",
  ...
  /* print colored output if activated in command line (-C). Based on model from
   * A. Diakite, TU-Berlin. Implemented by J. Wienold, August 26 2018 */
  ...
  if (color_output == 1) {
    ...
    printf("0\n");
    printf("22 %.3e %.3e %lf %lf %lf %lf %lf %f %f %f %f %f %f %f %f %f %f %f "
           "%f %f %f %f\n",
           diffnormalization, groundbr, *(c_perez + 0), *(c_perez + 1),
           *(c_perez + 2), *(c_perez + 3), *(c_perez + 4), sundir[0], sundir[1],
           sundir[2], skyclearness, locus[0], locus[1], locus[2], locus[3],
           locus[4], locus[5], locus[6], locus[7], locus[8], locus[9],
           locus[10]);
  } else {
    ...
    printf("10 %.3e %.3e %lf %lf %lf %lf %lf %f %f %f \n", diffnormalization,
           groundbr, *(c_perez + 0), *(c_perez + 1), *(c_perez + 2),
           *(c_perez + 3), *(c_perez + 4), sundir[0], sundir[1], sundir[2]);
  }
}
...
void usage_error(char *msg) /* print usage error and quit */
{
  ...
  fprintf(stderr, "			 or:	  [-W|-L|-G]  direct_value "
  ...
  fprintf(stderr, "	-G direct-horizontal-irradiance "
                  "diffuse-horizontal-irradiance (W/m^2)\n");
  fprintf(stderr, "	-E global-horizontal-irradiance (W/m^2)\n\n");
  fprintf(stderr, "	Output specification with option:\n");
  fprintf(stderr, "	-O [0|1|2]  (0=output in W/m^2/sr visible, 1=output in "
                  "W/m^2/sr solar, 2=output in candela/m^2), default is 0 \n");
...
double glob_h_effi_PEREZ() {
  ...
  /*if ((skyclearness<skyclearinf || skyclearness>skyclearsup ||
     skybrightness<skybriginf || skybrightness>skybrigsup) &&
     suppress_warnings==0) fprintf(stderr, "Warning: skyclearness or
     skybrightness out of range in function glob_h_effi_PEREZ \n"); */
  ...
  for (i = 1; i <= category_total_number; i++) {
    if ((skyclearness >= category_bounds[i]) &&
        (skyclearness < category_bounds[i + 1]))
      category_number = i;
  }

  value = a[category_number] + b[category_number] * atm_preci_water +
          c[category_number] * cos(sunzenith * M_PI / 180) +
...
double glob_h_diffuse_effi_PEREZ() {
  ...
  /*if ((skyclearness<skyclearinf || skyclearness>skyclearsup ||
   skybrightness<skybriginf || skybrightness>skybrigsup) &&
   suppress_warnings==0) fprintf(stderr, "Warning: skyclearness or skybrightness
   out of range in function glob_h_diffuse_PEREZ \n"); */
  ...
  if (category_number == -1) {
    if (suppress_warnings == 0)
      fprintf(stderr,
              "Warning: sky clearness (= %.3f) too high, printing error sky\n",
              skyclearness);
    print_error_sky();
    exit(0);
  }
...

{
  ...
  /*if ((skyclearness<skyclearinf || skyclearness>skyclearsup ||
     skybrightness<skybriginf || skybrightness>skybrigsup) &&
     suppress_warnings==0) fprintf(stderr, "Warning: skyclearness or
     skybrightness out of range in function direct_n_effi_PEREZ \n");*/
  ...
  value = a[category_number] + b[category_number] * atm_preci_water +
          c[category_number] * exp(5.73 * sunzenith * M_PI / 180 - 5) +
...
void check_illuminances() {
  ...
  if (directilluminance + diffuseilluminance == 0 && altitude > 0) {
    if (suppress_warnings == 0) {
      fprintf(stderr, "Warning: zero illuminance at sun altitude > 0, printing "
                      "error sky\n");
    }
    print_error_sky();
    exit(0);
...
void check_irradiances() {
  ...
  if (directirradiance + diffuseirradiance == 0 && altitude > 0) {
    if (suppress_warnings == 0) {
      fprintf(
          stderr,
          "Warning: zero irradiance at sun altitude > 0, printing error sky\n");
    }
    print_error_sky();
    exit(0);
  }
...
double sky_brightness() {
  ...
  value =
      diffuseirradiance * air_mass() / (solar_constant_e * get_eccentricity());
...
double sky_clearness() {
  ...
  value = ((diffuseirradiance + directirradiance) / (diffuseirradiance) +
           1.041 * sunzenith * M_PI / 180 * sunzenith * M_PI / 180 * sunzenith *
               M_PI / 180) /
          (1 + 1.041 * sunzenith * M_PI / 180 * sunzenith * M_PI / 180 *
                   sunzenith * M_PI / 180);
...
double direct_irradiance_from_sky_clearness() {
  ...
  value = value * ((skyclearness - 1) *
                   (1 + 1.041 * sunzenith * M_PI / 180 * sunzenith * M_PI /
                            180 * sunzenith * M_PI / 180));
...
void illu_to_irra_index() {
  ...
  diffuseirradiance =
      diffuseilluminance * solar_constant_e / (solar_constant_l);
  directirradiance = directilluminance * solar_constant_e / (solar_constant_l);
  skyclearness = sky_clearness();
  skybrightness = sky_brightness();
  check_parametrization();

  while (((fabs(diffuseirradiance - test1) > 10) ||
          (fabs(directirradiance - test2) > 10) ||
...
double calc_rel_lum_perez(double dzeta, double gamma, double Z, double epsilon,
  ...
  if (num_lin) {
    ...
    c_perez[3] =
        -exp(Delta * (x[3][0] + x[3][1] * Z)) + x[3][2] + Delta * x[3][3];
  }
...
void coeff_lum_perez(double Z, double epsilon, double Delta,
  ...
  if (num_lin) {
    ...
    *(c_perez + 4) = x[4][0] + x[4][1] * Z + Delta * (x[4][2] + x[4][3] * Z);
    *(c_perez + 2) =
        exp(pow(Delta * (x[2][0] + x[2][1] * Z), x[2][2])) - x[2][3];
    *(c_perez + 3) =
        -exp(Delta * (x[3][0] + x[3][1] * Z)) + x[3][2] + Delta * x[3][3];
  }
...
double radians(double degres) { return degres * (M_PI / 180.); }

/* radian into degrees */
double degres(double radians) { return radians * (180. / M_PI); }

/* calculation of the angles dzeta and gamma */
void theta_phi_to_dzeta_gamma(double theta, double phi, double *dzeta,
                              double *gamma, double Z) {
  *dzeta = theta; /* dzeta = phi */
  if ((cos(Z) * cos(theta) + sin(Z) * sin(theta) * cos(phi)) > 1 &&
      (cos(Z) * cos(theta) + sin(Z) * sin(theta) * cos(phi) < 1.1))
    *gamma = 0;
...
/* enter day number(double), return E0 = square(R0/R):  eccentricity correction
 * factor  */
...
double air_mass() {
  ...
  m = 1 / (cos(sunzenith * M_PI / 180) +
           0.15 * exp(log(93.885 - sunzenith) * (-1.253)));
  return (m);
}
