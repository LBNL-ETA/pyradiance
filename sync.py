...
def perform_sync_operations(
    ...
    for subdir_name in subdirectories_to_sync:
        ...
        pyradiance_target_subdir_rel_path = (
            Path(pyradiance_base_target_rel_path) / subdir_name
        )

        radiance_src_full_path = radiance_clone_root / radiance_src_subdir_rel_path
        pyradiance_target_full_path = (
            pyradiance_root / pyradiance_target_subdir_rel_path
        )

        print(f"\nProcessing subdirectory: '{subdir_name}'")
        print(
            f"  Radiance source      : {radiance_src_full_path} (relative to clone: {radiance_src_subdir_rel_path})"
        )
        print(
            f"  PyRadiance target    : {pyradiance_target_full_path} (relative to project: {pyradiance_target_subdir_rel_path})"
        )
        ...
        if not radiance_src_full_path.is_dir():
            print(
                f"  WARNING: Radiance source path '{radiance_src_full_path}' does not exist or is not a directory. Skipping this subdirectory."
            )
            ...
            if dry_run and not radiance_clone_root.exists():
                ...
                print(
                    f"    Please check if '{radiance_src_subdir_rel_path}' is a valid path in the Radiance repo structure."
                )
            continue
        ...
        if not dry_run:
            ...
            print(
                f"  DRY RUN: Would ensure target directory exists: {pyradiance_target_full_path}"
            )
        # --- MODIFICATION END ---
        ...
        if not sorted_source_files:
            print(
                f"  No files matched the patterns for '{subdir_name}' in source '{radiance_src_full_path}'. Target directory untouched."
            )
            continue

        for src_file_path in sorted_source_files:
            relative_path_in_source_subdir = src_file_path.relative_to(
                radiance_src_full_path
            )
            dest_file_path = (
                pyradiance_target_full_path / relative_path_in_source_subdir
            )
            ...
            if not dry_run:
                ...
                )  # copy2 preserves metadata and overwrites
            else:
                ...
                print(
                    f"    DRY RUN: Would {action.lower().split(' ')[0]}: {dry_run_src_display} -> {dry_run_dest_display}"
                )

        total_processed = copied_files_count + updated_files_count
        if total_processed > 0:
            print(
                f"  Processed {total_processed} files for '{subdir_name}': {copied_files_count} new, {updated_files_count} updated."
            )
...
def main_sync_orchestrator(
    pyradiance_root_arg, radiance_branch="master", dry_run=False
):
    """
        ...
        if not dry_run:
            run_command(
                [
                    ...
                    radiance_branch,
                    RADIANCE_REPO_URL,
                    str(radiance_clone_target_dir),
                ]
            )
        else:
            print(
                f"  DRY RUN: Would clone {RADIANCE_REPO_URL} (branch {radiance_branch}) to {radiance_clone_target_dir}"
            )
    ...
    if not dry_run:
        ...
        print(
            f"  Consider running 'git status' and 'git diff' in '{pyradiance_root_path}'."
        )
        print("  Remember: Files in target directories not matching source patterns,")
        print(
            "  or files from older Radiance versions removed in the current source, were NOT deleted."
        )
